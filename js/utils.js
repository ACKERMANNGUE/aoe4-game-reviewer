...
export function getRankIcon(rankLevel, type = 'solo') {
  if (!rankLevel) return null;
  const folder = type === 'team' ? 'ranks/teams' : 'ranks/solo';
  const prefix = type === 'team' ? 'team' : 'solo';
  return `${folder}/${prefix}_${rankLevel.toLowerCase()}.svg`;
}
